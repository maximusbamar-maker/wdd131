...
form.addEventListener("submit", function (event) {
    ...
    console.log(note)
    if (type === "many" && note === "") {
        output.textContent = "Please add a note to your response"
        return
    }
    ...
    if (type === "many" && selectedCampuses.length <= 1) {
        output.textContent = "Please choose at least two campuses for the option of serving at two or more campuses"
        return
    }

    // Let the user know if they choose a date that has already passed
