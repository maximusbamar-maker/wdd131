...
let recipeContainer = document.getElementById("recipies");

rand_int = Math.floor(Math.random() * recipes.length);
renderRecipie(recipes[rand_int]);
...
function renderRecipie(recipe) {

    let buttons = ``
    for (let i = 0; i < recipe.tags.length; i++) {
        buttons += `<button class="tag">${recipe.tags[i]}</button>`
    }
    ...
    let recipeCard = `<div class="recipe-card">
            <img class="picture" src="${recipe.image}" alt="${recipe.name}">
            <div class="tags">
                ${buttons}
            </div>
...
function search() {
    ...
    let filteredRecipies = recipes.filter(function(recipe) {
        tags = []
        for (let i = 0; i < recipe.tags.length; i++) {
            tags.push(recipe.tags[i].toLowerCase());
        }
    ...
    let sortedRecipes = filteredRecipies.sort(compareRecipies);

    recipeContainer.innerHTML = '';
    sortedRecipes.forEach(function(recipe){
        renderRecipie(recipe);
    })
}